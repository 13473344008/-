...
export function renderPassport(container,payload,options={}){
 ...
 function fields(parent,object,omit=[],group=''){const dl=el('dl',null,'fields');for(const [k,v]of Object.entries(object)){const target=({raw_material:{origin:'raw_material_origin',description:'raw_material_description'},packaging:{description:'package_description'},storage:{conditions:'storage_conditions'}})[group]?.[k];if(omit.includes(k)||((!present(v)||typeof v==='object'||typeof v==='boolean')&&(!target||!pictures(target).length)))continue;const f=el('div',null,'field');f.append(el('dt',t(k)),el('dd',typeof v==='string'&&['pass','fail','pending','not_tested','not_applicable','informational'].includes(v)?t(v):displayValue(k,v,language),/(code|date|number|value|limit|hash|unit|size|weight|quantity|version)/.test(k)?'technical':undefined));if(target)appendPictures(f,target);dl.append(f);}if(dl.childNodes.length)parent.append(dl);}
 for(const k of ['product','batch','raw_material'])if(Object.values(p[k]).some(present)||p.assets.some(a=>({raw_material:['raw_material_origin','raw_material_description'],packaging:['package_description'],storage:['storage_conditions']})[k]?.includes(a.display_target))){const c=card(t(k),k,['raw_material','manufacturer'].includes(k));fields(c,p[k],[],k);}
 if(p.process.length){
