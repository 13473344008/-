...
2. 在 `admin/go-admin` 中运行 `go run -tags sqlite3 . migrate -c config/settings.yml` 初始化数据库，再运行 `go run -tags sqlite3 . server -c config/settings.yml`。SQLite 必须带 `sqlite3` 构建标签。初始化后在本地设置管理员密码，勿将框架演示凭据用于生产。
...
- 测试记录批次编码需包含独立的 `TEST` 段，例如 `PRODUCT-TEST-20260911-001`。目前该条件在发布校验时执行，新建时的提前提示尚需完善。
- 管理界面统一名称为“数字身份证管理系统”。
