...
**Gate：40 / 40 PASS。Tests：239 PASS，0 FAIL。** 数量为最终命名断言/Go 子测试，不是全部独立 E2E。上游 UI 301 单元测试另行通过，不计入此业务总数。早期失败与修复保留证据，不声称所有尝试均成功。
...
| 测试组 | PASS | FAIL |
| --- | --- | --- |
| Go/GORM business tests | 61 | 0 |
| HTTP API / Casbin | 63 | 0 |
| Backend restart | 5 | 0 |
| Browser aggregate and permissions restart | 6 | 0 |
| Browser E2E | 39 | 0 |
| Migration | 10 | 0 |
| Fresh HTTP | 12 | 0 |
| Fresh browser | 10 | 0 |
| Final protection and integrity | 33 | 0 |

总计 239 PASS，0 FAIL。Go 编译、完整前端 type-check、全量 lint（0 errors，30 条上游 warnings）、check:i18n、Vite 构建通过；上游 UI 36 文件/301 tests 通过。没有升级依赖或为消除上游 warning 批量改文件。
...
- `admin/go-admin/cmd/t6readprobe/main.go`
- `admin/go-admin/app/passport/models/batch.go`
- `admin/go-admin/app/passport/service/dto/batch.go`
- `admin/go-admin/app/passport/service/batch_validation.go`
- `admin/go-admin/app/passport/service/batch.go`
- `admin/go-admin/app/passport/service/batch_test.go`
- `admin/go-admin/app/passport/service/batch_edges_test.go`
- `admin/go-admin/app/passport/apis/batch.go`
- `admin/go-admin/app/admin/router/passport_batches.go`
- `admin/go-admin/cmd/migrate/migration/version/1788912000000_batch_management.go`
- `admin/go-admin/cmd/migrate/migration/version/1788912000000_batch_management.sql`
- `admin/go-admin-ui/src/api/passport/batches.ts`
- `admin/go-admin-ui/src/views/passport/batches/index.vue`
- `admin/go-admin-ui/src/lang/zh-CN/passport/batches.ts`
- `admin/go-admin-ui/src/lang/en-US/passport/batches.ts`
- `admin/t6/build_report.py`
- `admin/t6/env.sh`
- `admin/t6/setup.py`
- `admin/t6/verify_api.py`
- `admin/t6/verify_browser.cjs`
- `admin/t6/verify_final.py`
- `admin/t6/verify_fresh_browser.cjs`
- `admin/t6/verify_fresh_http.py`
- `admin/t6/verify_migrations.py`
...
| 1 | Batch 列表真实可用。 | PASS |
| 2 | Batch 创建真实可用。 | PASS |
| 3 | Batch Code UNIQUE 应用层 + DB 层生效。 | PASS |
| 4 | 创建 Batch 自动绑定正确 Default Sealed Revision。 | PASS |
| 5 | 无有效 Default Revision 时创建被拒绝。 | PASS |
| 6 | Batch 创建后 base_product_revision_id 不可普通修改。 | PASS |
| 7 | Product 不可通过普通 Batch Edit 换绑。 | PASS |
| 8 | Product Default Revision 后续变化不会改变历史 Batch。 | PASS |
| 9 | 新 Batch 使用新的 Current Default Revision。 | PASS |
| 10 | 删除已被 Batch 引用 Revision 被阻止。 | PASS |
| 11 | Override inherit / set / clear 三态真实可用。 | PASS |
| 12 | Override 白名单 Backend 生效。 | PASS |
| 13 | 禁止字段 Override 负向测试通过。 | PASS |
| 14 | Reset Override 能真正恢复继承。 | PASS |
| 15 | Effective Resolver 结果正确。 | PASS |
| 16 | Inspection 动态项目真实可用。 | PASS |
| 17 | 新增未知 Inspection 项目无需 Schema 修改。 | PASS |
| 18 | Inspection 状态 Validation 生效。 | PASS |
| 19 | Batch 多表保存使用 Transaction。 | PASS |
| 20 | Transaction 失败完整 Rollback。 | PASS |
| 21 | Clone Batch 真实可用。 | PASS |
| 22 | Clone 默认保持源 Batch Base Revision。 | PASS |
| 23 | Clone 不错误复制旧批次实测结果。 | PASS |
| 24 | Clone Override 行为符合设计。 | PASS |
| 25 | Clone 多表写入使用 Transaction。 | PASS |
| 26 | 所有 Batch API 要求认证并走 Casbin。 | PASS |
| 27 | DTO / 字段白名单 / Mass Assignment 防护通过。 | PASS |
| 28 | Batch Audit 正常产生。 | PASS |
| 29 | Backend 重启后 Batch 数据完整。 | PASS |
| 30 | 浏览器完整 T6 流程通过。 | PASS |
| 31 | 并发创建不存在重复 Batch Code。 | PASS |
| 32 | Fresh DB 从零建立成功。 | PASS |
| 33 | Migration 可重复运行。 | PASS |
| 34 | integrity_check = ok。 | PASS |
| 35 | foreign_key_check 无异常。 | PASS |
| 36 | 原 53 个 T0 基线文件不变。 | PASS |
| 37 | site 7 个文件不变。 | PASS |
| 38 | 未开发正式审核发布流程。 | PASS |
| 39 | 未开发 Published Snapshot Engine。 | PASS |
| 40 | 未进入 T7。 | PASS |
...
| # | 问题 | 回答 |
| --- | --- | --- |
| 1 | Batch 列表是否真实可用？ | Yes；分页、批次编码搜索、产品/工作流筛选。 |
| 2 | Batch 创建是否真实可用？ | Yes；真实 API 与浏览器已验证。 |
| 3 | Batch Code 是否业务层校验 UNIQUE？ | Yes；trim/大写规范化，冲突返回 409。 |
| 4 | DB UNIQUE 是否仍生效？ | Yes；DB UNIQUE 负向通过。 |
| 5 | 创建 Batch 是否真实保存 base_product_revision_id？ | Yes；写入 Batch 行，不在读取时查当前默认。 |
| 6 | base_product_revision_id 是否来自正确 Product？ | Yes；应用检查加组合 FK。 |
| 7 | 默认是否只接受合法 Sealed Revision？ | Yes；同产品 sealed，且 Product active。 |
| 8 | 没有 Default Revision 是否拒绝创建？ | Yes；清晰拒绝，不选最新/R1 兜底。 |
| 9 | Draft Revision 是否不能作为 Base？ | Yes；应用与数据库都拒绝。 |
| 10 | 其他 Product 的 Revision 是否不能绑定？ | Yes；本阶段 Create DTO 不接受手选 base，DB 跨产品关系也拒绝。 |
| 11 | Batch 创建后能否通过普通 API 修改 Base Revision？ | No。 |
| 12 | Batch 创建后能否通过普通 API修改 Product？ | No。 |
| 13 | Product 默认 Revision 改变是否不会改变旧 Batch？ | Yes；旧批次保持 R1。 |
| 14 | 新 Batch 是否使用新 Default Revision？ | Yes；切默认后新批次绑定 R2。 |
| 15 | 删除被引用 Product Revision 是否被阻止？ | Yes；sealed 保护与 FK 保留。 |
| 16 | Override 是否支持 inherit？ | Yes；无行表示 inherit。 |
| 17 | Override 是否支持 set？ | Yes；按白名单类型 set。 |
| 18 | Override 是否支持 clear？ | Yes；遵循 T3 各字段 NULL/空数组规则。 |
| 19 | Reset 是否真正恢复 inherit？ | Yes；删除覆盖及译文，读取固定 base。 |
| 20 | Override 白名单是否 Backend 强制执行？ | Yes；拒绝未知、系统字段与未开放资产类型。 |
| 21 | batch_code 是否不能 Override？ | Yes。 |
| 22 | product_id 是否不能 Override？ | Yes。 |
| 23 | base_product_revision_id 是否不能 Override？ | Yes。 |
| 24 | workflow_status 是否不能 Override？ | Yes。 |
| 25 | clear 是否也有合法性检查？ | Yes；检查字段许可、allow_clear 和无附带值。T3 已开放的 13 个非资产字段均允许 clear。 |
| 26 | 是否存在统一 Effective Resolver？ | Yes；ResolveEffectiveBatch。 |
| 27 | 页面是否显示 inherited / overridden / cleared 来源？ | Yes；页面三种来源标签。 |
| 28 | Inspection 是否为动态模型？ | Yes；逐行 inspection_items。 |
| 29 | Bulk Density 是否无需 Schema 修改即可增加？ | Yes；服务和浏览器均新增。 |
| 30 | Black Specks 是否无需 Schema 修改即可增加？ | Yes；服务和浏览器均新增。 |
| 31 | Inspection Status Validation 是否生效？ | Yes；六个既有枚举，数值/状态一致性检查。 |
| 32 | Batch 保存多表操作是否使用 Transaction？ | Yes；一次 PUT 保存基础信息、覆盖、检测、Audit。 |
| 33 | Transaction 中途失败是否完整 Rollback？ | Yes；实际错误注入全回滚。 |
| 34 | Clone Batch 是否真实可用？ | Yes；仅未锁定可见 Draft，要求版本前置条件。 |
| 35 | Clone 是否生成新的 Batch ID？ | Yes；主/子行 UUID 全新。 |
| 36 | Clone 是否要求新的 Batch Code？ | Yes；用户输入新的稳定编码。 |
| 37 | Clone 默认是否保持源 Batch Base Revision？ | Yes；保持源 base。 |
| 38 | Clone 是否避免隐式切换到 Product 当前最新 Revision？ | Yes；不查当前默认来重绑。 |
| 39 | Clone 是否没有复制旧批次真实检测值？ | Yes；清空 numeric/text/result_display_text、检测日期及事实备注，判定 not_tested。 |
| 40 | Clone 是否正确复制允许的检测项目结构？ | Yes；复制代码/名称/单位/规格/限值/方法/排序/公开候选标志。 |
| 41 | Clone Override 是否符合已定义规则？ | Yes；保留允许覆盖和译文，译文状态回 draft，可 reset。 |
| 42 | Clone 多表写是否使用 Transaction？ | Yes。 |
| 43 | Clone 失败是否整体 Rollback？ | Yes；Inspection Copy 失败注入通过。 |
| 44 | Batch API 是否全部要求认证？ | Yes；全部 5 条 API 匿名拒绝。 |
| 45 | 是否全部经过 Casbin？ | Yes；JWT/AuthCheckRole/PermissionAction，上游 admin 特权逻辑保持。 |
| 46 | 是否使用明确 DTO？ | Yes；明确聚合 DTO。 |
| 47 | 是否防止 Mass Assignment？ | Yes；未知/重复字段和系统字段拒绝。 |
| 48 | Batch Audit 是否产生？ | Yes；追加到 passport_audit_events。 |
| 49 | Audit 是否尽量与业务修改同事务？ | Yes；本模块全部写操作与 Audit 同事务。 |
| 50 | Backend 重启后 Batch 数据是否保持？ | Yes；API 样本及浏览器完整聚合重启一致。 |
| 51 | 浏览器完整流程是否通过？ | Yes；39 项主流程通过。 |
| 52 | Default Revision 改变场景是否浏览器实测？ | Yes；实际 UI 切默认后核验旧 A/R1、新 B/R2。 |
| 53 | Clone Batch 是否浏览器实测？ | Yes；克隆后新测值保存刷新通过。 |
| 54 | 并发同 Batch Code 是否不会出现重复？ | Yes；5 个同码并发仅 1 成功，其余 409。 |
| 55 | 是否发生不可接受的 SQLite Lock？ | No；本次不同码、同码和 Clone 并发均无不可接受锁/超时；不代表生产无限容量。 |
| 56 | Fresh DB 是否能够从零建立？ | Yes；全 11 迁移、HTTP 和真实浏览器通过。 |
| 57 | Migration 是否可重复？ | Yes；升级库及空库重复执行保持数据/Schema。 |
| 58 | integrity_check 是否为 ok？ | Yes；主库/新库/服务测试库/升级副本均 ok。 |
| 59 | foreign_key_check 是否无异常？ | Yes；均无异常行。 |
| 60 | DATA_MODEL 是否发生变化？ | Yes；仅审计 event_type CHECK 新增 override_reset，19 表/308 列/ERD 关系不变。 |
| 61 | 如发生变化是否通过新增 Migration？ | Yes；1788912000000 新迁移。 |
| 62 | 是否修改 T4/T5 历史 Migration？ | No。 |
| 63 | 是否修改上游 tracked files？ | Yes；仍仅 UI 两个语言入口，后端 0。 |
| 64 | 如修改，具体哪些及原因？ | src/lang/zh-CN/index.ts、src/lang/en-US/index.ts：在 T5 基础上注册 passportBatch 语言包。 |
| 65 | 原 53 个基线文件 Hash 是否一致？ | Yes。 |
| 66 | site 7 个文件是否全部未变？ | Yes。 |
| 67 | 是否连接业务服务器？ | No。 |
| 68 | 是否修改 Caddy？ | No。 |
| 69 | 是否修改 Directus？ | No。 |
| 70 | 是否修改生产 Docker？ | No。 |
| 71 | 是否实现正式 Pending Review / Approval？ | No。 |
| 72 | 是否开发 Published Snapshot Engine？ | No。 |
| 73 | 是否开发正式 QR？ | No。 |
| 74 | 是否进入 T7？ | No。 |
