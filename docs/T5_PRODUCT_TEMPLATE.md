...
| 方法 | 路径 | 用途 |
| --- | --- | --- |
| GET | `/api/v1/passport-products` | 分页、search、status |
| POST | `/api/v1/passport-products` | 原子创建 Product/R1/翻译/审计 |
| GET | `/api/v1/passport-products/{id}` | Product 详情及 Revision History |
| PUT | `/api/v1/passport-products/{id}` | 仅 active/disabled |
| POST | `/api/v1/passport-products/{id}/archive` | 归档，保留历史 |
| GET | `/api/v1/passport-products/{id}/revisions` | 版本列表（含模板内容） |
| GET | `/api/v1/passport-products/{id}/revisions/{rid}` | 版本详情，含六语言和编辑 token |
| POST | `/api/v1/passport-products/{id}/revisions` | 基于 source_revision_id 创建下一版本 |
| PUT | `/api/v1/passport-products/{id}/revisions/{rid}` | Draft 内容替换，expected_token 必填 |
| PUT | `/api/v1/passport-products/{id}/revisions/{rid}/translations` | 单语言 upsert，expected_token 必填 |
| POST | `/api/v1/passport-products/{id}/revisions/{rid}/seal` | 封版，expected_token 必填 |
| PUT | `/api/v1/passport-products/{id}/default-revision` | 显式选择 revision_id，检查 expected_current_revision_id |
...
| 测试组 | 数量 | PASS | FAIL |
| --- | --- | --- | --- |
| Go/GORM Service | 27 | 27 | 0 |
| HTTP API and permissions | 50 | 50 | 0 |
| Backend restart | 5 | 5 | 0 |
| Browser E2E | 21 | 21 | 0 |
| Migrations | 10 | 10 | 0 |
| Fresh HTTP | 4 | 4 | 0 |
| Final integrity and baseline | 18 | 18 | 0 |
...
| 10 | 冻结翻译 API 拒绝 | PASS |
| 11 | Clone | PASS |
| 12 | Revision Number 唯一 | PASS |
| 13 | 显式默认切换 | PASS |
| 14 | 默认合法性 | PASS |
| 15 | 六语言 API | PASS |
| 16 | Translation 唯一 | PASS |
| 17 | Archive 保留历史 | PASS |
| 18 | 业务 API 认证 | PASS |
| 19 | 业务审计 | PASS |
| 20 | 重启持久化 | PASS |
| 21 | 浏览器全流程 | PASS |
| 22 | Fresh DB | PASS |
| 23 | integrity_check | PASS |
| 24 | foreign_key_check | PASS |
| 25 | 53 文件基线 | PASS |
| 26 | site 7 文件 | PASS |
...
- `admin/go-admin-ui/src/api/passport/products.ts`
- `admin/go-admin-ui/src/lang/en-US/passport/index.ts`
- `admin/go-admin-ui/src/lang/en-US/passport/products.ts`
- `admin/go-admin-ui/src/lang/zh-CN/passport/index.ts`
- `admin/go-admin-ui/src/lang/zh-CN/passport/products.ts`
- `admin/go-admin-ui/src/views/passport/products/index.vue`
- `admin/go-admin/app/admin/router/passport_products.go`
- `admin/go-admin/app/passport/apis/product.go`
- `admin/go-admin/app/passport/apis/strict.go`
- `admin/go-admin/app/passport/models/product.go`
- `admin/go-admin/app/passport/service/dto/product.go`
- `admin/go-admin/app/passport/service/product.go`
- `admin/go-admin/app/passport/service/product_test.go`
- `admin/go-admin/app/passport/service/validation.go`
- `admin/go-admin/cmd/migrate/migration/version/1788825600000_product_templates.go`
- `admin/go-admin/cmd/migrate/migration/version/1788825600000_product_templates.sql`
- `admin/t5/build_report.py`
- `admin/t5/env.sh`
- `admin/t5/setup.py`
- `admin/t5/verify_api.py`
- `admin/t5/verify_browser.cjs`
- `admin/t5/verify_final.py`
- `admin/t5/verify_fresh_http.py`
- `admin/t5/verify_migrations.py`
...
| # | 问题 | 回答 |
| --- | --- | --- |
| 1 | Products 列表真实可用？ | Yes；分页、编码/名称搜索、状态筛选及详情通过。 |
| 2 | Product 创建真实可用？ | Yes；同时建立 Product、R1、源语言翻译。 |
| 3 | 重复编码被业务层拒绝？ | Yes；trim/大写规范化后返回 409 和清晰提示。 |
| 4 | DB UNIQUE 最后保护？ | Yes；直接 SQL 重复编码/版本/翻译被拒。 |
| 5 | Product 与 Revision 真正分离？ | Yes；稳定 UUID 身份与独立多版本行。 |
| 6 | 初始 Revision 创建？ | Yes；Create Product 事务创建 R1 Draft。 |
| 7 | 三表创建使用 Transaction？ | Yes，另含业务 Audit。 |
| 8 | 中途失败全回滚？ | Yes；注入翻译 INSERT 错误及 Audit INSERT 错误均验证。 |
| 9 | Draft 可编辑？ | Yes；内容令牌检测旧稿冲突。 |
| 10 | 冻结主体 Backend 拒绝？ | Yes，409；DB 触发器也保留。 |
| 11 | 冻结 Translation Backend 拒绝？ | Yes，409；DB 触发器也保留。 |
| 12 | 新 Revision 不修改旧版？ | Yes，旧聚合逐字节比较通过。 |
| 13 | R1/R2 完整保留？ | Yes。 |
| 14 | Revision Number 不重复？ | Yes；取得写锁后事务分配，5 个并发请求生成 R3–R7。 |
| 15 | Clone 复制允许内容？ | Yes；结构化模板及全部已存在译文；未开放子系统存在时明确拒绝。 |
| 16 | Clone 系统字段重置？ | Yes；新 ID/actor/时间，draft、sealed_* / hash 清空；译文重置 draft。 |
| 17 | 默认 Revision 可设置？ | Yes，独立显式 API。 |
| 18 | 默认必须属于 Product？ | Yes。 |
| 19 | 默认状态校验？ | Yes，只允许 sealed。 |
| 20 | 切默认不修改历史？ | Yes；仅更新 products 指针和元数据，写审计，不写 Batch/旧 Revision。 |
| 21 | 六语言共存？ | Yes；en、zh-CN、es、ar、fr、de。 |
| 22 | 同版本同语言不能重复？ | Yes；API upsert，DB UNIQUE。 |
| 23 | Archive 不破坏历史？ | Yes；默认指针及所有版本保留。 |
| 24 | 普通 Hard Delete 破坏历史路径？ | 本模块没有 DELETE API，只有归档。 |
| 25 | 全部业务 API 要求认证？ | Yes；12 条路由匿名负向测试。 |
| 26 | 绕过 Casbin？ | No；使用上游 JWT、AuthCheckRole 和菜单/API 权限；保留上游 admin 特权逻辑。 |
| 27 | 使用 Request DTO/白名单？ | Yes；严格解码，拒绝未知和重复字段。 |
| 28 | 避免 Mass Assignment？ | Yes；系统 ID/actor/状态等不能通过内容 DTO 覆盖。 |
| 29 | 业务 Audit 产生？ | Yes；沿用既有审计表及合适事件，新增 7 个注册事件。 |
| 30 | Audit 与业务同事务？ | Yes，全部本模块写操作。 |
| 31 | 重启后业务数据存在？ | Yes；产品详情聚合与重启前完全一致。 |
| 32 | 浏览器完整流程？ | PASS，21 项断言。 |
| 33 | 并发版本无重复？ | PASS，5 个写入者全部成功，无重试。 |
| 34 | Fresh DB 从零建立？ | Yes；全部 10 个迁移后登录并创建产品。 |
| 35 | Migration 重复运行？ | Yes；升级库及 Fresh 库重复执行不改变数据/Schema。 |
| 36 | integrity_check=ok？ | Yes。 |
| 37 | foreign_key_check？ | PASS，无异常行。 |
| 38 | 修改 DATA_MODEL Schema？ | 仅扩展审计 event_type CHECK；业务 19 表/308 列及关系不变。 |
| 39 | 通过新 Migration？ | Yes，1788825600000；T4 历史迁移未改。 |
| 40 | 修改上游 tracked files？ | Yes，UI 2 文件；Backend 0 文件。 |
| 41 | 具体哪些及原因？ | src/lang/zh-CN/index.ts、src/lang/en-US/index.ts，仅注册新增 passport 语言包。 |
| 42 | 53 基线 Hash 一致？ | Yes。 |
| 43 | site 7 文件未变？ | Yes。 |
| 44 | 连接业务服务器？ | No。 |
| 45 | 修改 Caddy？ | No。 |
| 46 | 修改 Directus？ | No。 |
| 47 | 修改生产 Docker？ | No。 |
| 48 | 开发 Batch 正式管理？ | No。 |
| 49 | 开发 Published Snapshot Engine？ | No。 |
| 50 | 进入 T6？ | No。 |
