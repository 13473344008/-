<template>
  <PageContainer>
    ...
    >
      ...
      <el-table-column :label="$t('traffic.region')" prop="loginLocation" min-width="180" show-overflow-tooltip>
        <template #default="{ row }">{{ row.loginLocation === 'Local network' ? $t('traffic.local') : !row.loginLocation || row.loginLocation === 'Unknown' ? $t('traffic.unknown') : row.loginLocation }}</template>
      </el-table-column>
      <!--
