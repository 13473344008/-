<template>
  <PageContainer>
    ...
    >
      ...
      <el-table-column :label="$t('traffic.region')" prop="operLocation" min-width="180" show-overflow-tooltip>
        <template #default="{ row }">{{ row.operLocation === 'Local network' ? $t('traffic.local') : !row.operLocation || row.operLocation === 'Unknown' ? $t('traffic.unknown') : row.operLocation }}</template>
      </el-table-column>
