<template>
  <el-card v-loading="busy" class="section-manager" data-testid="section-manager">
    ...
    <div class="section-toolbar">
      ...
      <el-button v-permisaction="['passport:sections:write']" :disabled="readonly || busy || editing" data-testid="section-specs" @click="startSpecs">{{ t('passportSections.addSpecs') }}</el-button>
      <el-select v-model="previewLanguage" :disabled="editing" @change="load"><el-option v-for="l in languages" :key="l" :value="l" :label="t(`passportSections.language_${l}`)" /></el-select>
    </div>
    ...
    <el-dialog v-model="editing" :title="t('passportSections.editor')" :width="form.section_type === 'table' ? 'min(1280px, 95vw)' : 'min(900px, 95vw)'" :close-on-click-modal="false" :before-close="close" destroy-on-close>
      <el-form label-position="top" :disabled="readonly || busy" data-testid="section-editor">
        ...
        <div class="section-grid">
          <el-form-item v-if="advanced" :label="t('passportSections.key')"><el-input v-model="form.section_key" :disabled="!!editingId || inherited" maxlength="64" data-testid="section-key" /></el-form-item>
          <el-form-item :label="t('passportSections.type')"><el-select v-model="form.section_type" :disabled="!!editingId || inherited" data-testid="section-type" @change="changeType"><el-option v-for="kind in types" :key="kind" :value="kind" :label="t(`passportSections.type_${kind}`)" /></el-select></el-form-item>
          <el-form-item v-if="advanced" :label="t('passportSections.order')"><el-input-number v-model="form.sort_order" :min="0" :max="100000" data-testid="section-order" /></el-form-item>
        ...
        <el-checkbox v-if="advanced" v-model="form.is_visible" :disabled="inherited && !baseRow(form.section_key)?.allow_hide">{{ t('passportSections.visible') }}</el-checkbox>
        <el-checkbox v-model="form.is_public">{{ t('passportSections.public') }}</el-checkbox>
        <el-checkbox v-if="advanced && !batch" v-model="form.allow_hide" data-testid="section-allow-hide">{{ t('passportSections.allowHide') }}</el-checkbox>
        <el-tabs v-model="language"><el-tab-pane v-for="l in languages" :key="l" :name="l" :label="t(`passportSections.language_${l}`)" /></el-tabs>
          ...
          <template v-else>
            ...
            <template v-if="form.section_type === 'key_value'">
              <div v-for="(item, i) in translation.content.items" :key="i" class="section-grid">
                <el-input v-if="advanced" v-model="item.key" :placeholder="t('passportSections.key')" :disabled="language !== set?.source_language" @change="align" />
                ...
                <el-button @click="removeItem(i)">{{ t('common.delete') }}</el-button>
              </div>
...
function addItem() { const key = `item_${crypto.randomUUID().replaceAll('-', '')}`; form.value.translations.forEach(x => x.content.items?.push({ key, label: '', value: '' })) }
function removeItem(i: number) { form.value.translations.forEach(x => x.content.items?.splice(i, 1)) }
function addColumn() { const key = `column_${crypto.randomUUID().replaceAll('-', '')}`; form.value.translations.forEach(x => { x.content.columns?.push({ key, label: '' }); x.content.rows?.forEach(row => row.cells.push('')) }) }
function removeColumn(i: number) { form.value.translations.forEach(x => { x.content.columns?.splice(i, 1); x.content.rows?.forEach(row => row.cells.splice(i, 1)) }) }
function addRow() { form.value.translations.forEach(x => x.content.rows?.push({ cells: x.content.columns?.map(() => '') ?? [] })) }
function removeRow(i: number) { form.value.translations.forEach(x => x.content.rows?.splice(i, 1)) }
function startSpecs() {
  start()
  form.value.section_type = 'table'
  form.value.translations = languages.map(l => ({ language_code: l, translation_status: 'draft', title: t('passportSections.specsTitle', {}, { locale: l === 'en' ? 'en-US' : l }), content: { columns: ['parameter', 'specification', 'unit', 'method'].map(key => ({ key, label: t(`passportSections.column_${key}`, {}, { locale: l === 'en' ? 'en-US' : l }) })), rows: [{ cells: ['', '', '', ''] }] }}))
