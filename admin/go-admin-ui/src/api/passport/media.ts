...
export const listMedia = (base: string, display_target?: string) => request<ApiResponse<MediaSet>>({ url: base, timeout: 30000, params: display_target === undefined ? undefined : { display_target }})
export const uploadMedia = (base: string, file: File, expectedToken: string, label: string, isPublic: boolean, displayTarget = '') => {
  ...
  return request<ApiResponse<{ id: string }>>({ url: base, method: 'post', data, timeout: 90000 })
}
export const detachMedia = (base: string, id: string, expected_token: string) => request<ApiResponse<null>>({ url: `${base}/${id}`, method: 'delete', timeout: 30000, data: { expected_token }})
