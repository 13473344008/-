...
const MODEL_DIRS = [
  ...
  'app/jobs/models',
  'app/passport/models',
  'app/passport/service', // scheduled jobs
...
// Passport pages render multiple explicitly typed server rows. Bind them to their
// actual module, avoiding the demo Product alias and guessed DTO filenames.
const PASSPORT_PAGES = {
  'passport/products/index.vue': { models: ['ProductRow', 'RevisionView'], dto: 'product.go' },
  'passport/batches/index.vue': { models: ['BatchRow', 'AuditItem'], dto: 'batch.go' },
  'passport/reviews/index.vue': { models: ['ReviewQueueRow'], dto: 'review.go' }
}
const PAGES = migratedPages()

for (const [page, model] of Object.entries(PAGES)) {
  const text = readFileSync(join(UI, 'src/views', page), 'utf8')
  const binding = PASSPORT_PAGES[page]
  const fields = binding ? Object.assign({}, ...binding.models.map(m => fieldsOf(m))) : fieldsOf(model)
  if (binding && binding.models.some(m => !Object.keys(fieldsOf(m)).length)) problems.push(`${page}\n    explicitly bound Go model missing`)
  ...
  const accepted = formTagsOf(binding?.dto ?? `${snakeCase(model)}.go`)
