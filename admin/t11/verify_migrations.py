...
source=R/'runtime/t10/db/passport-admin-t10.db';up=rt/'db/upgrade-from-t10.db';fresh=rt/'fresh/db/fresh-acceptance.db';service=rt/'db/service-test.db'
...
def migrate(db,name,port=18107):
 cfg=rt/(name+'.yml');cfg.write_text((rt/'settings.yml').read_text().replace(str(rt/'db/passport-admin-t11.db'),str(db)).replace('port: 18107','port: '+str(port)));cfg.chmod(0o600)
 ...
 return cfg
before=snap(up);migrate(up,'upgrade');check('T10 all original business columns preserved',snap(up)==before);state=structure(up);migrate(up,'upgrade-repeat');check('Upgrade repeated safe',structure(up)==state and snap(up)==before)
cfg=migrate(fresh,'fresh',18108);check('Fresh equals upgrade schema',structure(fresh)==state);migrate(fresh,'fresh-repeat',18108);check('Fresh repeat schema stable',structure(fresh)==state)
with sqlite3.connect(fresh) as c:check('Fresh has sixteen migrations',c.execute('select count(*) from sys_migration').fetchone()[0]==16);check('Fresh business starts empty',all(c.execute('select count(*) from '+t).fetchone()[0]==0 for t in tables))
