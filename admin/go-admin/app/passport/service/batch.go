...
const batchSelect = `batches.current_review_record_id,batches.id,batches.created_at,batches.created_by,batches.updated_at,batches.updated_by,batches.batch_code,batches.product_id,batches.base_product_revision_id,batches.record_type,batches.production_date,batches.expiry_date,batches.quality_status,batches.workflow_status,batches.edit_version,batches.submitted_edit_version,batches.submitted_content_hash,batches.submitted_schema_version,batches.submitted_builder_version,batches.submitted_preview_hash,batches.submitted_at,batches.submitted_by,batches.reviewed_at,batches.reviewed_by,batches.rejection_reason,batches.current_passport_revision_id,batches.active_publish_record_id,batches.next_version_number,batches.cloned_from_batch_id,batches.archived_at,batches.archived_by,batches.internal_note, CASE WHEN batches.workflow_status='pending_review' AND EXISTS(SELECT 1 FROM review_records rr WHERE rr.id=batches.current_review_record_id AND rr.decision='approved') THEN 'ready_for_publish' ELSE batches.workflow_status END AS review_state, (SELECT product_code FROM products WHERE id=batches.product_id) AS product_code,
