...
func TestT12FixOrdinaryPublishFaultRecovery(t *testing.T) {
	dbpath := os.Getenv("T12_FIX_TEST_DB")
	fixtures := os.Getenv("T12_FIX_FIXTURES")
	if dbpath == "" || fixtures == "" {
		t.Fatal("dedicated T12-fix paths required")
	}
	...
	for _, stage := range []string{"build", "write", "copy1", "copy2", "validation", "manifest", "rename", "finalize"} {
		t.Run(stage, func(t *testing.T) {
			...
			t.Logf("stage=%s reached=true status=%s old_db_current=%s", want, failed.Record.PublishStatus, v1.Revision.ID)
