...
INSERT INTO t10_audit_events SELECT * FROM passport_audit_events;
DROP TABLE passport_audit_events;
ALTER TABLE t10_audit_events RENAME TO passport_audit_events;
...
ALTER TABLE publish_records ADD COLUMN rollback_reason TEXT CHECK(rollback_reason IS NULL OR (length(trim(rollback_reason)) BETWEEN 1 AND 1000 AND instr(rollback_reason,'<')=0 AND instr(rollback_reason,'>')=0));
DROP INDEX ux_review_published_once;
CREATE UNIQUE INDEX ux_review_published_once ON passport_revisions(source_review_record_id) WHERE published_at IS NOT NULL AND source_review_record_id IS NOT NULL AND rollback_source_revision_id IS NULL;
CREATE TRIGGER rollback_reason_insert BEFORE INSERT ON publish_records WHEN (NEW.operation_type='rollback' AND (NEW.rollback_reason IS NULL OR NOT EXISTS(SELECT 1 FROM passport_revisions WHERE id=NEW.passport_revision_id AND rollback_source_revision_id IS NOT NULL))) OR (NEW.operation_type='publish' AND NEW.rollback_reason IS NOT NULL) BEGIN SELECT RAISE(ABORT,'invalid rollback provenance'); END;
CREATE TRIGGER rollback_reason_fixed BEFORE UPDATE ON publish_records WHEN NEW.rollback_reason IS NOT OLD.rollback_reason BEGIN SELECT RAISE(ABORT,'rollback reason immutable'); END;
CREATE TABLE publication_health(batch_id TEXT PRIMARY KEY REFERENCES batches(id) ON DELETE RESTRICT, reconciliation_required INTEGER NOT NULL DEFAULT 0 CHECK(reconciliation_required IN(0,1)), classification TEXT NOT NULL, checked_at TEXT NOT NULL);
