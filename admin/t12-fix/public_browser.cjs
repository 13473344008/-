...
const fs=require('fs'),path=require('path');const rt=path.resolve(__dirname,'../../runtime/t12-fix'),base='http://127.0.0.1:19544',ui='http://127.0.0.1:19543',phase=process.argv[2]||'v1';
...
try{
 ...
 if(['v1','v2','rollback','stopped'].includes(phase)){
  await passport(base+'/b/PF-T12-TEST-001/v/1');await version(1);await passport();
 }
 if(['v1','v2','full','stopped'].includes(phase)){
 ...
 if(phase==='full'){
  ...
  for(const [path,status]of [['/b/NO-SUCH-TEST',404],['/b/PF-T12-TEST-DRAFT',404],['/b/PF-T12-TEST-001/v/99',404],['/runtime/t12-fix/credentials.json',404]]){const r=await page.goto(base+path);check('Nginx '+path,r.status()===status);}
...
}catch(e){checks.push({name:'Completed '+phase,status:'FAIL',error:e.message});console.error(e);process.exitCode=1;if(page)await page.screenshot({path:rt+'/test-artifacts/failure-'+phase+'.png',fullPage:true});}finally{fs.writeFileSync(rt+'/test-artifacts/public-browser-'+phase+'.json',JSON.stringify({checks,errors,network},null,2));await browser.close();}})();
