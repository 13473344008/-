...
def get(path,headers={}):
 c=http.client.HTTPConnection('127.0.0.1',19544);c.request('GET',path,headers=headers);r=c.getresponse();body=r.read();out=(r.status,dict((k.lower(),v) for k,v in r.getheaders()),body);c.close();return out
try:
 ...
 for path in ['/versions/PF-T12-TEST-001/v1.json']+['/'+a['path'] for a in p['assets']]:
