...
def prepare():
 ...
 pid=json.loads(statefile.read_text())['product_id'];p='passport-products/'+pid;rev=ok(p)['revisions'][0];rid=rev['id'];rp=p+'/revisions/'+rid
 ok(rp,{'expected_token':ok(rp)['token'],'content':content},'PUT','editor');ok(rp+'/translations',{'expected_token':ok(rp)['token'],'translation':translation('en')},'PUT','editor')
 ...
 save({'product_id':pid,'r1':rid,'section_ids':ids,'versions':[]});check('Fresh Draft ready for real UI media upload',True)
