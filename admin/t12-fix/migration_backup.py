from helpers import *
import subprocess
src=rt/'db/passport-admin-t12-fix.db'
def snapshot(p):
...
with sqlite3.connect(src) as c:
 check('17 migrations applied',c.execute('select count(*) from sys_migration').fetchone()[0]==17)
 ...
 backup=rt/'backups/t12-fix-working-backup.db';restored=rt/'db/t12-fix-working-restored.db';started=datetime.datetime.now(datetime.timezone.utc).isoformat()
