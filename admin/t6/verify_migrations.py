...
R=Path(__file__).resolve().parents[2];rt=R/'runtime/t6';checks=[];tables=json.loads((R/'admin/t4/schema_catalog.json').read_text())['tables']
def check(n,v):
...
def migrate(db,name):
 cfg=rt/(name+'.yml');cfg.write_text((rt/'settings.yml').read_text().replace('passport-admin-t6.db',db.name));cfg.chmod(0o600)
 with (rt/'logs'/(name+'.log')).open('w') as log:subprocess.run([str(rt/'go-admin'),'migrate','-c',str(cfg)],cwd=R/'admin/go-admin',stdout=log,stderr=subprocess.STDOUT,check=True)
up=rt/'db/upgrade-from-t5.db';fresh=rt/'db/fresh-acceptance.db'
assert not up.exists() and not fresh.exists(),'Preserve existing migration evidence; use new dedicated paths'
# T5 was stopped. Immutable read-only connection avoids touching its WAL/SHM/evidence.
a=sqlite3.connect('file:'+str(R/'runtime/t5/db/passport-admin-t5.db')+'?mode=ro&immutable=1',uri=True);b=connect(up);a.backup(b);a.close();b.close();before=snap(up)
migrate(up,'migrate-upgrade');check('T5 populated schema upgrades without losing 19-table data',snap(up)==before)
with connect(up) as c:
 check('T6 version recorded once',c.execute("select count(*) from sys_migration where version='1788912000000'").fetchone()[0]==1)
...
with connect(fresh) as c:
 check('Fresh business tables empty',all(c.execute('select count(*) from '+t).fetchone()[0]==0 for t in tables));check('Fresh all 11 versions recorded',c.execute('select count(*) from sys_migration').fetchone()[0]==11);check('Fresh integrity and FK',c.execute('pragma integrity_check').fetchall()==[('ok',)] and c.execute('pragma foreign_key_check').fetchall()==[])
