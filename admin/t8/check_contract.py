...
import re,json
R=Path(__file__).resolve().parents[2];backend=R/'admin/go-admin/app/passport';front=R/'admin/go-admin-ui/src/api/passport/reviews.ts';structs={};checks=[]
...
for ts,go in [('QueueRow','ReviewQueueRow'),('Review','ReviewView'),('ReviewDetail','ReviewDetail'),('Readiness','Readiness'),('Candidate','ReviewCandidate')]:
...
(R/'runtime/t8/test-artifacts/contracts.json').write_text(json.dumps(checks,indent=2));print(len(checks),'PASS; upstream generic check is separately reported')
