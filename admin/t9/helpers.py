...
R=Path(__file__).resolve().parents[2];mode=sys.argv[1] if len(sys.argv)>1 else 'initial';rt=R/('runtime/t9/fresh' if mode=='fresh' else 'runtime/t9');cred=json.loads((rt/'credentials.json').read_text());checks=[];base='http://127.0.0.1:'+('18104' if mode=='fresh' else '18103')+'/api/v1/';admin=None;runid=datetime.datetime.now().strftime('%H%M%S');tokens={};users={}
...
def db():
 c=sqlite3.connect(rt/'db'/('fresh-acceptance.db' if mode=='fresh' else 'passport-admin-t9.db'));c.execute('pragma foreign_keys=on');return c
def scalar(sql,args=()):
 with db() as c:return c.execute(sql,args).fetchone()[0]
def login(username,password):
 x=req('login',{'username':username,'password':password,'code':'0','uuid':'T9'},token=None);assert x.get('code')==200,x;return x['token']
def product(code,token='editor'):
 p=ok('passport-products',{'product_code':code,'content':{'source_language':'en','process_steps':[]},'translations':[{'language_code':'en','translation_status':'approved','product_name':'T9 TEST ONLY','process_labels':{}}]},token=token)['id'];r=ok('passport-products/'+p,token=token)['revisions'][0];ok(f'passport-products/{p}/revisions/{r["id"]}/seal',{'expected_token':r['token']},token=token);v=ok('passport-products/'+p,token=token);ok(f'passport-products/{p}/default-revision',{'revision_id':r['id'],'expected_current_revision_id':None},'PUT',token);return p,r['id']
def batch(pid,code,token='editor',dates=True):
 return ok('passport-batches',{'product_id':pid,'batch_code':code,'record_type':'test','content':{'production_date':'2026-09-09' if dates else None,'expiry_date':'2027-09-09' if dates else None,'quality_status':'pending','internal_note':'T9 TEST'},'overrides':[],'inspections':[]},token=token)['id']
...
def sample(key='module'):
 return {'section_key':key,'operation':'add','section_type':'text','sort_order':1,'is_visible':True,'is_public':False,'allow_hide':False,'status':'draft','translations':[{'language_code':'en','translation_status':'draft','title':'T9 TEST module','content':{'text':'T9 frozen custom text'}}]}
def snap():
