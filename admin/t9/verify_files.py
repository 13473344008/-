...
R=Path(__file__).resolve().parents[2];rt=R/'runtime/t9';checks=[];counts={}
...
for root,dbfile in [(rt,'passport-admin-t9.db'),(rt/'fresh','fresh-acceptance.db')]:
 ...
 check(root.name+' one successful revision per review',c.execute('select count(*) from (select source_review_record_id from passport_revisions where published_at is not null group by source_review_record_id having count(*)>1)').fetchone()[0]==0)
 ...
 check(root.name+' no injected DB triggers',c.execute("select count(*) from sqlite_master where type='trigger' and name like 't9_fault%'").fetchone()[0]==0)
 ...
 for p in revisions:
  ...
  if p['published_at'] is not None:
   ...
   check('success audit '+p['id'],c.execute("select count(*) from passport_audit_events where event_type='publish_succeeded' and json_extract(after_data,'$.publish_record_id')=?",(rec['id'],)).fetchone()[0]==1)
 for b in c.execute('select b.batch_code,b.workflow_status,b.active_publish_record_id,p.snapshot_path,p.payload_hash from batches b join passport_revisions p on b.current_passport_revision_id=p.id'):
  check('head '+b['batch_code'],sha(root/'publish/published'/f'{b["batch_code"]}.json')==b['payload_hash'] and b['workflow_status']=='published' and b['active_publish_record_id'] is None)
 ...
 counts[root.name]={'published_revisions':published,'sealed_revisions':len(revisions),'published_asset_rows':assetcount,'records':c.execute('select count(*) from publish_records').fetchone()[0],'failed_records':c.execute("select count(*) from publish_records where publish_status='failed'").fetchone()[0],'tables':20,'business_columns':sum(len(c.execute('pragma table_info('+t+')').fetchall()) for t in list(json.loads((R/'admin/t4/schema_catalog.json').read_text())['tables'])+['review_records']),'triggers':c.execute("select count(*) from sqlite_master where type='trigger'").fetchone()[0]};c.close()
...
old=json.loads((rt/'test-artifacts/protected-before.json').read_text());changed=[p for p,h in old.items() if not (R/p).exists() or sha(R/p)!=h];check('36375 historical files including T4-T8 migrations unchanged',len(old)==36375 and not changed)
