...
const root=path.resolve(__dirname,'../..'),schema=JSON.parse(fs.readFileSync(root+'/runtime/t9/test-artifacts/public-schema-1.0.json'));
...
for(const dir of ['runtime/t9/publish/versions','runtime/t9/fresh/publish/versions']){
...
fs.writeFileSync(root+'/runtime/t9/test-artifacts/independent-schema.json',JSON.stringify({validator:'Ajv 6.15.0; fixed keyword subset supplemental check',checks},null,2));if(checks.some(x=>x.status==='FAIL'))process.exitCode=1;console.log(checks.length+' snapshots checked');
