...
source=R/'runtime/t8/db/passport-admin-t8.db';up=rt/'db/upgrade-from-t8.db';fresh=rt/'fresh/db/fresh-acceptance.db';service=rt/'db/service-test.db'
...
def migrate(db,name,port=18103):
 cfg=rt/(name+'.yml');cfg.write_text((rt/'settings.yml').read_text().replace(str(rt/'db/passport-admin-t9.db'),str(db)).replace('port: 18103','port: '+str(port)));cfg.chmod(0o600)
 ...
 return cfg
before=snap(up);migrate(up,'upgrade');check('T8 all original business columns preserved',snap(up)==before);state=structure(up);migrate(up,'upgrade-repeat');check('Upgrade repeated safe',structure(up)==state and snap(up)==before)
cfg=migrate(fresh,'fresh',18104);check('Fresh equals upgrade schema',structure(fresh)==state);migrate(fresh,'fresh-repeat',18104);check('Fresh repeat schema stable',structure(fresh)==state)
with sqlite3.connect(fresh) as c:check('Fresh has fourteen migrations',c.execute('select count(*) from sys_migration').fetchone()[0]==14);check('Fresh business starts empty',all(c.execute('select count(*) from '+t).fetchone()[0]==0 for t in tables))
