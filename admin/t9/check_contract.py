...
import re,json
R=Path(__file__).resolve().parents[2];backend=R/'admin/go-admin/app/passport';front=R/'admin/go-admin-ui/src/api/passport/publication.ts';structs={};checks=[]
...
for ts,go in [('PublishedRevision','PassportRevision'),('PublishEntry','PublishEntry'),('PublicationStatus','PublicationStatus'),('PublishRequest','PublishRequest')]:
...
(R/'runtime/t9/test-artifacts/contracts.json').write_text(json.dumps(checks,indent=2));print(len(checks),'PASS; upstream generic check is separately reported')
