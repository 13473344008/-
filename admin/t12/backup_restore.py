...
R=Path(__file__).resolve().parents[2];rt=R/'runtime/t12';checks=[]
...
started=datetime.datetime.now(datetime.timezone.utc).isoformat();source=rt/'db/passport-admin-t12.db';backup=rt/'backups/t12-final.db';restored=rt/'db/restored.db';before=snapshot(source)
...
for name,src in [('publish',rt/'publish'),('private-releases',rt/'releases'),('public-site',R/'public-site')]:
 dest=rt/'backups'/('final-'+name);shutil.copytree(src,dest);check('Backup bytes '+name,hashes(src)==hashes(dest))
 if name!='private-releases':
