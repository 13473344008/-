...
with sqlite3.connect(rt/'db/passport-admin-t12.db') as c:
...
for label,file in [('T4-T11 protection','protected-before.json'),('53 baseline','baseline-before.json')]:
 hashes=json.loads((rt/'test-artifacts'/file).read_text());check(label,all((r/p).is_file() and hashlib.sha256((r/p).read_bytes()).hexdigest()==h for p,h in hashes.items()))
