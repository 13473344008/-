...
const fs=require('fs'),path=require('path');const rt=path.resolve(__dirname,'../../runtime/t12'),base='http://127.0.0.1:19542',ui='http://127.0.0.1:19541',phase=process.argv[2]||'v1';
...
async function passport(url=base+'/b/PF-T12-TEST-001'){await page.goto(url);await expect(page.locator('[data-module=publication]')).toBeVisible();}
...
try{
 await passport();await version(phase==='v2'?2:['rollback','stopped'].includes(phase)?3:1);check('TEST notice visible',await page.locator('.test-notice').textContent()==='TEST RECORD — NOT FOR COMMERCIAL USE');
 if(['v1','v2','rollback','stopped'].includes(phase)){
  for(const n of (phase==='stopped'?[1,2,3]:[1])){await passport(base+'/b/PF-T12-TEST-001/v/'+n);await version(n);}await passport();
 }
 if(phase==='full'||phase==='stopped'){
  ...
  for(const module of ['product','batch','raw_material','process','inspection','packaging','storage','manufacturer','custom-text','custom-key_value','custom-table','custom-asset_gallery'])check('Rendered '+module,await page.locator('[data-module="'+module+'"]').count()>=1);
  ...
  check('Five dynamic inspections',await page.locator('.inspection-item').count()===5);
  ...
  check('Text-only fixture explicitly has no media',await page.locator('[data-module=assets] img').count()===0);
 }
 if(phase==='full'){
  const current=fs.readFileSync(rt+'/publish/published/PF-T12-TEST-001.json','utf8');
  async function bad(name,body,expected='Passport temporarily unavailable') {await page.route('**/published/PF-T12-TEST-001.json',r=>r.fulfill({contentType:'application/json',body}));await page.goto(base+'/b/PF-T12-TEST-001');await expect(page.locator('h1')).toHaveText(expected);check(name);await page.unroute('**/published/PF-T12-TEST-001.json');}
  ...
  await page.route('**/published/PF-T12-TEST-001.json',r=>r.abort());await page.goto(base+'/b/PF-T12-TEST-001');await expect(page.locator('h1')).toHaveText('Passport temporarily unavailable');check('Network failure finite error');await page.unroute('**/published/PF-T12-TEST-001.json');await page.getByText('Retry',{exact:true}).click();await expect(page.locator('[data-module=publication]')).toBeVisible();check('Retry recovers');
  ...
  for(const [path,status]of [['/b/NO-SUCH-TEST',404],['/b/PF-T12-TEST-DRAFT',404],['/b/PF-T12-TEST-001/v/99',404],['/runtime/t12/credentials.json',404]]){const r=await page.goto(base+path);check('Nginx '+path,r.status()===status);}
