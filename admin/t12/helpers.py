...
R=Path(__file__).resolve().parents[2];mode=sys.argv[1] if len(sys.argv)>1 else 'initial';rt=R/('runtime/t12/fresh' if mode=='fresh' else 'runtime/t12');cred=json.loads((rt/'credentials.json').read_text());checks=[];base='http://127.0.0.1:'+('18110' if mode=='fresh' else '18109')+'/api/v1/';admin=None;runid=datetime.datetime.now().strftime('%H%M%S');tokens={};users={}
...
def db():
 c=sqlite3.connect(rt/'db'/('fresh-acceptance.db' if mode=='fresh' else 'passport-admin-t12.db'));c.execute('pragma foreign_keys=on');return c
def scalar(sql,args=()):
