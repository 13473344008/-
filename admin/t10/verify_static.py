...
for p in sorted(root.rglob('*')):
 ...
 with urllib.request.urlopen('http://127.0.0.1:19538/'+rel) as f:raw=f.read();mime=f.headers.get_content_type()
 ...
 if p.suffix=='.json':
  ...
  for a in data['assets']:
   with urllib.request.urlopen('http://127.0.0.1:19538/'+a['path']) as f:b=f.read()
...
for rel in ['published/TEST-ABSENT.json','assets/sha256/aa/'+'a'*64+'.png','../private-media/','%2e%2e/settings.yml','tmp/','']:
 try:urllib.request.urlopen('http://127.0.0.1:19538/'+rel);status=200
