...
def init():
 ...
 for kind in ['editor','reviewer','viewer','editor_reviewer']:
  username='t10-'+kind+'-'+runid;role=scalar('select role_id from sys_role where role_key=?',('passport_'+kind,))
  ok('sys-user',{'username':username,'password':cred['test_password'],'nickName':'T10 '+kind,'phone':'00000000000','email':'t10@example.invalid','deptId':1,'postId':1,'roleId':role,'status':'2'})
...
def png(color):
 ...
 return b'\x89PNG\r\n\x1a\n'+chunk(b'IHDR',struct.pack('>IIBBBBB',2,2,8,2,0,0,0))+chunk(b'tEXt',b'Comment\x00PRIVATE-METADATA-T10')+chunk(b'IDAT',zlib.compress(b'\0'+bytes(color)*2+b'\0'+bytes(color)*2))+chunk(b'IEND',b'')
def asset(bid,public=True,index=1):
 section=sample('gallery'+str(index));section.update(section_type='asset_gallery',is_public=public,status='ready');section['translations']=[{'language_code':'en','translation_status':'approved','title':'T10 test image','content':{'caption':'T10 controlled test fixture'}}]
 ...
 with db() as c:
  ...
  c.execute('INSERT INTO asset_links(id,created_at,created_by,updated_at,updated_by,custom_section_id,media_asset_id,asset_key,asset_role,public_label,is_public) VALUES(?,?,?,?,?,?,?,?,?,?,?)',(str(uuid.uuid4()),now,uid,now,uid,sid,aid,'image'+str(index),'section_image','T10 test image',int(public)))
...
def approved(pid,suffix,assets=False):
 b=batch(pid,'TEST-T10-'+suffix+'-'+runid);a=asset(b) if assets else None;submit(b);assert decision(b).get('code')==200;return b,a

def main():
 init();pid,rid=product('TEST-P-T10-'+runid);bid=batch(pid,'TEST-T10-MAIN-'+runid);path='passport-batches/'+bid
 ...
 w=work(bid);w['inspections']=[{'item_code':'PRIVATE_TEST','name':'PRIVATE-INSPECTION-T10','value_type':'text','text_value':'PRIVATE-RESULT','judgement':'informational','min_inclusive':True,'max_inclusive':True,'sort_order':1,'is_public':False}]
