"""Summarize existing evidence; does not execute tests or change acceptance status."""
import json,hashlib
from pathlib import Path
R=Path(__file__).resolve().parents[2];rt=R/'runtime/t4';ev=rt/'evidence';read=lambda n:json.loads((ev/n).read_text());env=read('environment.json');backup=read('backup-restore.json');final=read('final-tests.json');schema=read('schema-tests.json');con=next(x['detail'] for x in schema if x['name']=='10 writers / 50 transactions');go=[json.loads(x) for x in (ev/'go-tests.jsonl').read_text().splitlines()];gocases=[{'name':x['Test'],'status':'PASS' if x['Action']=='pass' else 'FAIL'} for x in go if x.get('Action') in ('pass','fail') and '/' in x.get('Test','')];suites={'Schema / Python SQLite':schema,'首次 API / RBAC':read('api-initial.json'),'重启 API / RBAC':read('api-restart.json'),'UI':read('ui-tests.json')['checks'],'Fresh DB':read('fresh-tests.json')['checks'],'Backup / Restore':backup['checks'],'最终基线 / Schema / 停止核验':final['checks'],'Go/GORM 实际驱动':gocases};alltests=[x for s in suites.values() for x in s];assert len(alltests)==99 and all(x['status']=='PASS' for x in alltests)
questions=[
('Go Admin Backend 是否成功运行？','Yes；实际启动并重启，现已停止。'),('Go Admin UI 是否成功运行？','Yes；Vite 启动，现已停止。'),('UI 是否成功连接 Backend？','Yes；浏览器登录及三类管理页面通过。'),('是否真实使用 SQLite？','Yes；Go 实际驱动 SQLite 3.53.4。'),('SQLite 文件具体位置？',str(rt/'db/passport-admin-validated.db')),('官方 Migration 是否成功？','Yes；官方 7 个版本通过。'),('系统表是否建立？','Yes；21 张上游表（含迁移、Casbin 和演示表），另加 19 张业务表。'),('JWT 登录是否成功？','Yes；签发、getinfo 验证、非法 token 拒绝均通过。'),('RBAC / Casbin 是否持久化？','Yes；真实 API 创建并持久化。'),('重启后用户、角色、权限是否存在？','Yes；重新登录并复验允许/拒绝接口。'),('19 张业务表是否真实建立？','Yes；308 列与 T3 字段目录一致。'),('Product Code UNIQUE？','PASS。'),('Batch Code UNIQUE？','PASS。'),('Product Revision UNIQUE？','PASS。'),('Passport Revision Version UNIQUE？','PASS。'),('Translation UNIQUE？','PASS。'),('Foreign Key 是否开启？','Yes；Go 与 Python 实际连接 foreign_keys=1。'),('删除被引用 Product Revision 是否阻止？','Yes；冻结保护拒绝，外键及禁止级联同时保留。'),('Batch 是否固定绑定 Revision？','Yes；绑定 R1，重绑被拒绝。'),('R2 是否不改变历史 Batch？','Yes。'),('Override 三态？','Yes；无行继承 25、set 为 20、clear 为 NULL，移除覆盖恢复继承；非法字段被拒绝。'),('Inspection 新增未知项目无需 ALTER？','Yes；七项目含 Bulk Density，schema_version 不变。'),('六语言共存？','Yes；en、zh-CN、es、ar、fr、de。'),('V1/V2/V3/V4 回滚模型？','Yes；V4 的 rollback_source_revision_id=V2，V1–V3 保留。'),('Published Asset 保留 image-A？','Yes；冻结副本路径及实际 SHA-256 不变。'),('Media Asset 变化不影响历史资产？','Yes；另建 B，A disabled，删除 A 被 FK 拒绝，V1 A 副本可读。'),('Transaction Rollback？','PASS；Python 四表故意唯一冲突全回滚，正常四表全提交；Go 四表回滚补验通过。'),('并发写入结果？','Python：10 写入者/50 事务，50 成功；Go：5 写入者/25 事务，25 成功。'),('是否发生 database is locked？','最终两组测试均未发生。'),('锁是否可接受及可处理？','本次未触发锁超时；不能据此证明所有负载都不锁。后续保持短事务并设计有限重试。'),('Backup 是否实际成功？','Yes；SQLite Backup API。'),('Restore 是否实际成功？','Yes；停止服务后恢复至独立 restored.db，Go 驱动也成功读取。'),('Restore 数据完整？','Yes；备份时全部 40 表行数及逻辑内容 SHA-256 一致，含用户、角色、权限及核心业务记录。'),('integrity_check=ok？','Yes；主库、恢复库、Fresh 库、Go 补验库均通过。'),('foreign_key_check？','PASS；无异常行。'),('Fresh DB 从零建立？','Yes；删除专用 disposable 数据库后由所有迁移重建，未复制演示 DB。'),('Migration 可重复？','Yes；官方迁移重跑、全迁移 fresh 重跑均通过。'),('是否必须大量修改 Go Admin？','No；上游已跟踪文件改动 0，仅新增业务迁移与技术探针。'),('SQLite 当前是否推荐？','继续使用，限已确定的少量内部用户、公开静态化场景。'),('立即切换 PostgreSQL 的证据？','未发现；本轮没有安装或连接 PostgreSQL。'),('是否修改 site？','No；7 文件不变。'),('53 文件 SHA-256 是否一致？','Yes；开始与结束均一致。'),('是否连接业务服务器？','No。'),('是否修改 Caddy？','No。'),('是否修改 Directus？','No。'),('是否修改 Docker 生产环境？','No。'),('是否进入 T5？','No。')]
qtable='\n'.join(f'| {i} | {q} | {a} |' for i,(q,a) in enumerate(questions,1))
gates=['Backend 使用 SQLite 启动','UI 连接后端并登录','官方空库 Migration','重启后 RBAC / Casbin','T3 19 表建立','主要 FK / UNIQUE','Batch 固定 Revision','Passport 历史版本','Published Asset 冻结','Transaction Rollback','低并发写入','Backup','Restore','Integrity Check','Fresh DB 重建','无需大量侵入修改框架']
gatetable='\n'.join(f'| {i} | {g} | PASS |' for i,g in enumerate(gates,1))
testtable='\n'.join(f'| {n} | {len(v)} | {len(v)} | 0 |' for n,v in suites.items())
newfiles=sorted(str(p.relative_to(R)) for p in (R/'admin/t4').glob('*') if p.is_file())+['admin/.gitignore','runtime/.gitignore','admin/go-admin/cmd/t4probe/main.go','admin/go-admin/cmd/t4probe/validation_test.go']+sorted(str(p.relative_to(R)) for p in (R/'admin/go-admin/cmd/migrate/migration/version').glob('178873920*'))
text=f'''# 【T4 执行结果】
...
- {env['go']}；Node {env['node']}；pnpm {env['pnpm']}；{env['git']}；系统 SQLite CLI 3.51.0。
...
```text
DSN=file:{rt}/db/passport-admin-validated.db?_foreign_keys=on&_journal_mode=WAL&_busy_timeout=5000&_synchronous=FULL
foreign_keys=1
journal_mode=wal
busy_timeout=5000
synchronous=2 (FULL)
...
Python 事务跨 Batch / Inspection / Override / Audit：故意重复检测项目导致全部回滚，正常分支全部提交。Python 10 写入者执行 50 事务，成功 50、失败 0、重试 0，最大事务等待+执行 {con['max_latency_ms']} ms；核对 50 Batch + 50 Inspection + 50 Audit。
...
- Backup 时间：{backup['utc']}（UTC）。
- 文件：`{backup['backup_file']}`。
- 文件 SHA-256：`{backup['backup_sha256']}`。
- Restore：`{backup['restore_file']}`。
...
'''
(R/'docs/T4_RUNTIME_VALIDATION.md').write_text(text)
(R/'docs/T4_TEST_RESULTS.json').write_text(json.dumps({'status':'等待人工验收','total':99,'pass':99,'fail':0,'suites':suites,'gates':[{'gate':i,'name':g,'result':'PASS'}for i,g in enumerate(gates,1)],'entered_T5':False},indent=2,ensure_ascii=False))
print('Report: 99 PASS; 16 gates; 47 answers')
